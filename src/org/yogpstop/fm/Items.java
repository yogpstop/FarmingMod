...
public class Items extends Item {
	...
	public String getTextureFile() {
		return "/org/yogpstop/fm/items.png";
	}
	...
	public String getItemNameIS(ItemStack is) {
		return ims.get(is.getItemDamage()).getItemName();
	}
	...
	public boolean onItemUse(ItemStack is, EntityPlayer ep, World world, int x,
		...
		return ims.get(is.getItemDamage()).onItemUse(is, ep, world, x, y, z,
				direction, par8, par9, par10);
	}

	@Override
	public ItemStack onFoodEaten(ItemStack is, World world, EntityPlayer ep) {
		return ims.get(is.getItemDamage()).onFoodEaten(is, world, ep);
	}
	...
	public void getSubItems(int par1, CreativeTabs ct, List list) {
		...
		for (Integer j : i) {
			if (j != null) {
				...
				if (im.getMaxStackSize() != this.getItemStackLimit())
					continue;
				...
				if (label.equals("foodmaterials") && !im.isPlantable()
						&& !im.isCookware() && !im.isEatable()) {
					list.add(new ItemStack(par1, 1, j));
				}
	...
	public CreativeTabs[] getCreativeTabs() {
		return new CreativeTabs[] { FarmingMod.tabCookwares,
				FarmingMod.tabMaterials, FarmingMod.tabPlants,
