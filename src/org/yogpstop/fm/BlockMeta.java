...
public class BlockMeta extends Block {
	...
	public ArrayList<ItemStack> getBlockDropped(World world, int x, int y,
		...
		for (Drop d : drops) {
			a.add(d.getDrop((byte) world.getBlockMetadata(x, y, z),
					new Random()));
		}
		return a;
	}
	...
	private boolean checkBlock(World world, int x, int y, int z, boolean place) {
		if (plantTo.contains(world.getBlockId(x, y - 1, z))
				&& ((world.getBlockMaterial(x, y - 1, z) != Material.water) || (world
						.getBlockMetadata(x, y - 1, z) == 0))
				&& (!(place ? hasWater : continuousWater) || (world
						.getBlockMaterial(x - 1, y - 1, z) == Material.water
						|| world.getBlockMaterial(x + 1, y - 1, z) == Material.water
						|| world.getBlockMaterial(x, y - 1, z - 1) == Material.water || world
						.getBlockMaterial(x, y - 1, z + 1) == Material.water))
				&& minLight <= world.getBlockLightValue(x, y, z)
				&& world.getBlockLightValue(x, y, z) <= maxLight) {
			return true;
		}
		return false;
	}
	...
	public int getBlockTextureFromSideAndMetadata(int side, int meta) {
		return textureStart + meta;
	}

	@Override
	public boolean canPlaceBlockAt(World world, int x, int y, int z) {
		if (checkBlock(world, x, y, z, true)) {
			return true;
		}
		return false;
	}
